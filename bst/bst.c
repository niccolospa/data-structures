...
BST BSTinit(){
	...
	bst->head = NEW(ITEMsetvoid(), NULL, NULL, NULL, 0);
	bst->z = bst->head;
	bst->N = 0;
	return bst;
}
...
void BSTpreorder(BST bst){
	preorderR(bst->head, bst->z);
}
...
void BSTpostorder(BST bst){
	postorderR(bst->head, bst->z);
}
